...
<div class="max-w-2xl mx-auto">
    <h1 class="text-2xl font-extrabold text-[#1B3B2F] mb-1">Edit Product</h1>
    <p class="text-sm text-stone-500 mb-6">Changes are resubmitted for admin review before going live again.</p>
    ...
    <form action="{{ route('vendor.products.update', $product) }}" method="POST" enctype="multipart/form-data"
        ...
        <div>
            ...
            <textarea name="description" rows="3"
                      class="w-full rounded-xl border-stone-300 focus:border-[#2F6B4F] focus:ring-[#2F6B4F]">{{ old('description', $product->description) }}</textarea>
        ...
        <div>
            <label class="block text-sm font-semibold text-stone-700 mb-1">Current photos</label>
            ...
            <label for="images" class="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-stone-300 rounded-2xl py-6 cursor-pointer hover:border-[#2F6B4F] hover:bg-[#F7F3E8] transition">
                <span class="text-xl">📤</span>
                <span class="text-sm font-semibold text-[#2F6B4F]" id="upload-cta">Click to replace all photos (optional)</span>
                <span class="text-xs text-stone-400">Leave empty to keep the current photos — JPG/PNG up to 4MB each</span>
            </label>
...
(function () {
    ...
    function syncInputFiles() {
        ...
        cta.textContent = selectedFiles.length
            ? `${selectedFiles.length} of ${MAX_PHOTOS} new photos selected — click to add more`
            : 'Click to replace all photos (optional)';
    }
